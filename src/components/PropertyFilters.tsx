...
const PropertyFilters = ({ data, filters, onFilterChange, onReset }: PropertyFiltersProps) => {
  ...
  const propertyTypes = React.useMemo(() => {
    return [...new Set(data.map(p => p.property_type))].filter(Boolean).sort();
  }, [data]);
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          ...
          <div className="space-y-2 col-span-full">
            ...
            <div className="flex flex-wrap gap-2">
              ...
              {districts.length > 10 && (
                <Button variant="outline" size="sm" onClick={() => setShowAllDistricts(!showAllDistricts)}>
                  {showAllDistricts ? "Show less" : `+${districts.length - 10} more`}
                </Button>
              )}
            </div>
            {showAllDistricts && (
